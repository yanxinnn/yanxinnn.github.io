...
function setup() {
  ...
  acornSlides = [acornLogo, acornBottles];
...
function draw() {
  ...
  if (abs(acornstation.position.x - player.position.x) <= 180) {
    if (keyPressed() == "E") {
      slideShow("acorn", acornSlides, "About");
  ...
  if (abs(timeinterface.position.x - player.position.x) <= 100) {
    if (keyPressed() == "E") {
      slideShow("Time Interface for LinkNYC", acornSlides, "About");
      if (sound.checked) { continueSound = true; }
      else { continueSound = false; }
      slideShowing = true;
    }
  ...
  if (abs(missilecommand.position.x - player.position.x) <= 100) {
    if (keyPressed() == "E") {
      slideShow("Missile Command Clone", acornSlides, "About");
      if (sound.checked) { continueSound = true; }
      else { continueSound = false; }
      slideShowing = true;
    }
  ...
  if (abs(wizardsjourney.position.x - player.position.x) <= 100) {
    if (keyPressed() == "E") {
      slideShow("Wizard's Journey", acornSlides, "About");
      if (sound.checked) { continueSound = true; }
      else { continueSound = false; }
      slideShowing = true;
    }
...
function slideShow(title, slideName, title1) {
  snd_windyPetals.stop();
  moveSpeed = 0;
  player.changeAnimation("idle");
  ...
  // Acorn Station
  document.getElementById("slides").src = acornSlides[0];
}
